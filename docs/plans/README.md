...
| [0024](0024-foobar-component-version-single-source.md) | Single-source the foobar component version + refresh stale plugin descriptions | approved | foobar's Components list shows the plugin as a hardcoded `0.1.0` (never bumped since Plan 0001) while the app is `0.7.0`. Make the component version **track the workspace `[workspace.package].version`** via a **generated header**: `build.ps1` reads root `Cargo.toml` and writes `plugin-foobar/build/foo_lmv_version.h` (`#define FOO_LMV_VERSION`), which `foo_lmv.cpp` includes (guarded, with a `0.0.0-dev` fallback) and hands to `DECLARE_COMPONENT_VERSION`. **Revises Plan 0006's "plugin version remains independent" note** — the component version is no longer an independent axis, but the **C ABI axis (`LMV_ABI_VERSION`) stays independent**. Phase 2 refreshes the two stale description strings still naming the culled spectrum/pulse/starfield scenes (Plan 0012). Plugin + build-script + docs only; **no core, no C ABI change, no new tracked file** (header is gitignored). [ADR-0025](../adrs/0025-foobar-component-version-single-sourced.md); rejected a `/D` cl string-define (quote-escaping fragility), a committed `version.h` (not single-sourced), and a hand-bumped independent version. Independent of everything else in flight; can land anytime. |
